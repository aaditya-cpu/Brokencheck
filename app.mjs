...
async function runLighthouseDesktop(domain) {
  try {
    ...
    const issues = lighthouseResult.audits['diagnostics']?.details?.items.map(item => item.description).join(', ') || 'No issues';
...
// Main function to loop through all domains and process both Linkinator and Lighthouse results
